...
#define RIGHT_SENSOR_OFFSET 4
#define MIDDLE_SENSOR_OFFSET 8
#define LEFT_SENSOR_OFFSET 12
...
void enable_sensor_a(void){
    ...
    void* sensor_base = map + (ADDR_SENSOR & MAP_MASK);
    void* sensor_enables = sensor_base + ENABLE_OFFSET;
    *((uint32_t*)sensor_enables) = 1 | *((uint32_t*)sensor_enables) & 7;  // 001
    munmap(map, MAP_SIZE);
    close(fd);
}
...
void enable_sensor_b(void){
    ...
    void* sensor_base = map + (ADDR_SENSOR & MAP_MASK);
    void* sensor_enables = sensor_base + ENABLE_OFFSET;
    *((uint32_t*)sensor_enables) = 2 | *((uint32_t*)sensor_enables) & 7;  // 010
    munmap(map, MAP_SIZE);
    close(fd);
}
...
void enable_sensor_c(void){
    ...
    void* sensor_base = map + (ADDR_SENSOR & MAP_MASK);
    void* sensor_enables = sensor_base + ENABLE_OFFSET;
    *((uint32_t*)sensor_enables) = 4 | *((uint32_t*)sensor_enables) & 7;  // 100
    munmap(map, MAP_SIZE);
    close(fd);
}
